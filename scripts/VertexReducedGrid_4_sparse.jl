...
fermiGrid = sort(union(-nFermi:spacing:nFermi-1, [-nFermi,-1,0,nFermi-1]))
boseGrid = sort(union(-nBose:spacing:nBose-1, [-nBose,-1,0,nBose]))
freqList = [(i,j,k) for i in boseGrid for j in fermiGrid .- trunc(Int64,shift*i/2) for k in fermiGrid .- trunc(Int64,shift*i/2)]
const freqList_int = map(x->tripleToInt(x..., nBh,nB,nB2), freqList)
const len_freq = length(freqList)
